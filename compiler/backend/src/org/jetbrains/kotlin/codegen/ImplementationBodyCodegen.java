...
public class ImplementationBodyCodegen extends ClassBodyCodegen {
    ...
    private void generateSecondaryConstructorImpl(
        ...
        // Delegated constructor is always resolved for secondary constructors
        ResolvedCall<ConstructorDescriptor> constructorDelegationCall =
                getDelegationConstructorCallWithAssert(bindingContext, constructorDescriptor);
        ConstructorDescriptor delegatedConstructor = constructorDelegationCall.getResultingDescriptor();

        generateDelegatorToConstructorCall(iv, codegen, constructorDescriptor, constructorDelegationCall);
        if (!isSameClassConstructor(delegatedConstructor)) {
    ...
    private void generateDelegatorToConstructorCall(
        ...
        iv.load(0, OBJECT_TYPE);
        ConstructorDescriptor delegatingConstructor = delegationConstructorCall.getResultingDescriptor();

        CallableMethod delegatingCallable = typeMapper.mapToCallableMethod(delegatingConstructor);
        CallableMethod callable = typeMapper.mapToCallableMethod(constructorDescriptor);

        List<JvmMethodParameterSignature> delegatingParameters = delegatingCallable.getValueParameters();
        List<JvmMethodParameterSignature> parameters = callable.getValueParameters();

        ArgumentGenerator argumentGenerator;
        if (isSameClassConstructor(delegatingConstructor)) {
            // if it's the same class constructor we should just pass all synthetic parameters
            argumentGenerator =
                    generateThisCallImplicitArguments(iv, codegen, delegatingConstructor, delegatingCallable, delegatingParameters,
                                                      parameters);
        }
        else {
            argumentGenerator =
                    generateSuperCallImplicitArguments(iv, codegen, constructorDescriptor, delegatingConstructor, delegatingCallable,
                                                       delegatingParameters,
                                                       parameters);
        }

        codegen.invokeMethodWithArguments(
                delegatingCallable, delegationConstructorCall, StackValue.none(), codegen.defaultCallGenerator, argumentGenerator);
    }

    private boolean isSameClassConstructor(ConstructorDescriptor delegatingConstructor) {
        return delegatingConstructor.getContainingDeclaration() == descriptor;
    }
    ...
    private static ArgumentGenerator generateThisCallImplicitArguments(
        ...
        if (index == delegatingParameters.size() && index < parameters.size()) {
            assert parameters.get(index).getKind() == JvmMethodParameterKind.VALUE :
